...
class WalletManager {
    ...
    async init() {
        try {
            ...
            // Prefer the global attached to `window` to avoid ReferenceErrors for
            // an unbound identifier. If it's missing, try dynamically importing
            // the ESM bundle and attaching it to window.
            if (!window.HashConnect) {
                console.log('window.HashConnect not found, attempting to import module...');
                await this.loadHashConnect();
            }

            if (!window.HashConnect) {
                throw new Error('HashConnect library is not available');
            }

            // Construct the HashConnect instance using the exported constructor.
    ...
    async loadHashConnect() {
        return new Promise((resolve, reject) => {
            // Use dynamic ESM import so we pick up the exported symbol and attach it to window.
            // This avoids issues where the distributed file is an ES module and doesn't
            // expose a global when loaded via a classic <script> tag.
            (async () => {
                try {
                    const mod = await import('https://cdn.jsdelivr.net/npm/hashconnect@3.0.14/dist/hashconnect.js');
                    if (mod && mod.HashConnect) {
                        window.HashConnect = mod.HashConnect;
                        resolve();
                    } else {
                        reject(new Error('Imported module did not contain HashConnect export'));
                    }
                } catch (err) {
                    reject(err);
                }
